...
function observeLines(readable) {
  ...
  const scan$ = Rx.fromEvent(readable, 'data').pipe((0, _operators.scan)(({ buffer }, chunk) => {
    buffer += chunk;
  ...
  scan$.pipe((0, _operators.mergeMap)(({ lines }) => lines)),

  // inject the "unsplit" data at the end
  scan$.pipe((0, _operators.last)(), (0, _operators.mergeMap)(({ buffer }) => buffer ? [buffer] : []),
