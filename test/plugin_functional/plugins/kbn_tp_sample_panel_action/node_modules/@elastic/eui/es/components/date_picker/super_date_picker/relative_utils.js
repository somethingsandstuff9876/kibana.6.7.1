...
export function parseRelativeParts(value) {
  var matches = _.isString(value) && value.match(/now(([\-\+])([0-9]+)([smhdwMy])(\/[smhdwMy])?)?/);
...
export function toRelativeStringFromParts(relativeParts) {
  var count = _.get(relativeParts, "count", 0);

  var isRounded = _.get(relativeParts, "round", false);

  if (count === 0 && !isRounded) {
    return 'now';
  }

  var matches = _.get(relativeParts, "unit", 's').match(/([smhdwMy])(\+)?/);
